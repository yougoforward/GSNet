...
__all__ = ['gsnet6Net', 'get_gsnet6net']


class gsnet6Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(gsnet6Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = gsnet6NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class gsnet6NetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(gsnet6NetHead, self).__init__()
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_gsnet6(x)
...
def gsnet6Conv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class gsnet6Pooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(gsnet6Pooling, self).__init__()
...
class gsnet6_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = gsnet6Conv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = gsnet6Conv(in_channels, out_channels, rate2, norm_layer)
...
def get_gsnet6net(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = gsnet6Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
