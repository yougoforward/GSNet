...
__all__ = ['gsnet8Net', 'get_gsnet8net']


class gsnet8Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(gsnet8Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = gsnet8NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class gsnet8NetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(gsnet8NetHead, self).__init__()
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_gsnet8(x)
...
def gsnet8Conv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class gsnet8Pooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(gsnet8Pooling, self).__init__()
...
class gsnet8_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = gsnet8Conv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = gsnet8Conv(in_channels, out_channels, rate2, norm_layer)
...
def get_gsnet8net(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = gsnet8Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
