...
__all__ = ['new_gsnet2Net', 'get_new_gsnet2net']


class new_gsnet2Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(new_gsnet2Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = new_gsnet2NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class new_gsnet2NetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
        ...
        self.aa_new_gsnet2 = new_gsnet2_Module(in_channels, inter_channels, atrous_rates, norm_layer, up_kwargs)
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_new_gsnet2(x)
...
def new_gsnet2Conv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class new_gsnet2Pooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(new_gsnet2Pooling, self).__init__()
...
class new_gsnet2_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = new_gsnet2Conv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = new_gsnet2Conv(in_channels, out_channels, rate2, norm_layer)
        self.b3 = new_gsnet2Conv(in_channels, out_channels, rate3, norm_layer)
...
def get_new_gsnet2net(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = new_gsnet2Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
