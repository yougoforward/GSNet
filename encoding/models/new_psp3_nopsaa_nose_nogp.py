...
__all__ = ['new_psp3_nopsaa_nose_nogpNet', 'get_new_psp3_nopsaa_nose_nogpnet']


class new_psp3_nopsaa_nose_nogpNet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(new_psp3_nopsaa_nose_nogpNet, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = new_psp3_nopsaa_nose_nogpNetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class new_psp3_nopsaa_nose_nogpNetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(new_psp3_nopsaa_nose_nogpNetHead, self).__init__()
        self.se_loss = se_loss
        inter_channels = in_channels // 4

        self.aa_new_psp3_nopsaa_nose_nogp = new_psp3_nopsaa_nose_nogp_Module(in_channels, inter_channels, atrous_rates, norm_layer, up_kwargs)
        self.conv8 = nn.Sequential(nn.Dropout2d(0.1), nn.Conv2d(inter_channels, out_channels, 1))
        if self.se_loss:
            self.selayer = nn.Linear(inter_channels, out_channels)

    def forward(self, x):
        feat_sum = self.aa_new_psp3_nopsaa_nose_nogp(x)
...
def new_psp3_nopsaa_nose_nogpConv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class new_psp3_nopsaa_nose_nogpPooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(new_psp3_nopsaa_nose_nogpPooling, self).__init__()
...
class new_psp3_nopsaa_nose_nogp_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        super(new_psp3_nopsaa_nose_nogp_Module, self).__init__()
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = new_psp3_nopsaa_nose_nogpConv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = new_psp3_nopsaa_nose_nogpConv(in_channels, out_channels, rate2, norm_layer)
        self.b3 = new_psp3_nopsaa_nose_nogpConv(in_channels, out_channels, rate3, norm_layer)
...
def get_new_psp3_nopsaa_nose_nogpnet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = new_psp3_nopsaa_nose_nogpNet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
